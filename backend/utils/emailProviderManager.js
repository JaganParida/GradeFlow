...
let cachedBrevo1Transporter = null;

function getBrevo1Transporter() {
  if (cachedBrevo1Transporter) return cachedBrevo1Transporter;
  ...
  cachedBrevo1Transporter = nodemailer.createTransport({
    host,
    port,
    secure,
...
let cachedBrevo2Transporter = null;

function getBrevo2Transporter() {
  if (cachedBrevo2Transporter) return cachedBrevo2Transporter;
  ...
  if (!emailUser || !emailPass) {
    return null; // Secondary Brevo account not configured
  }

  const host = process.env.BREVO_HOST_2 || process.env.EMAIL_HOST || "smtp-relay.brevo.com";
  const port = Number(process.env.BREVO_PORT_2 || process.env.EMAIL_PORT) || 587;
  const secure = port === 465;

  cachedBrevo2Transporter = nodemailer.createTransport({
    host,
    port,
    secure,
    auth: { user: emailUser, pass: emailPass },
...
function resetTransporterCache() {
  cachedBrevo1Transporter = null;
  cachedBrevo2Transporter = null;
  cachedGmailTransporter = null;
}

/**
 * Dispatches an email with automatic Brevo #1 -> Brevo #2 -> Gmail fallback.
...
async function sendMailWithFailover(mailOptions) {
  ...
  if (isBrevo1Available) {
    try {
      const result = await brevo1.sendMail(mailOptions);
      providerState.brevo1.dailySentCount += 1;
      providerState.brevo1.consecutiveFailures = 0;
      return { success: true, provider: "brevo1", messageId: result?.messageId };
    } catch (err) {
      brevo1Failed = true;
      ...
      console.warn(
        `[EmailService] Brevo #1 delivery failed (${brevo1Classification}): ${err.message}`
      );
  ...
  if (brevo1Failed) {
    ...
    if (isBrevo2Available) {
      try {
        const result = await brevo2.sendMail(mailOptions);
        ...
        console.warn(
          `[EmailService] Brevo #2 delivery failed (${brevo2Classification}): ${err.message}`
        );
  ...
  if (brevo1Failed && brevo2Failed) {
    ...
    if (isGmailAvailable) {
      try {
        ...
        const result = await gmail.sendMail(fallbackOptions);
