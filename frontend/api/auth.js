...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    /* ═══════════════════════════════════════════════════════════════════
       1B. STUDENT RECOVERY OTP DISPATCH
    ═══════════════════════════════════════════════════════════════════ */
    if ((action === "student-send-recovery-otp" || action === "send-recovery-otp") && req.method === "POST") {
    ...
    if ((action === "admin-check-status" || action === "check-admin-status") && req.method === "GET") {
      ...
      let resolvedButtonVisible = activeSessions.length < MAX_ADMIN_DEVICES || isCurrentDevice;
      if (buttonVisibilityConfig && buttonVisibilityConfig.mode === "MANUAL") {
        const roles = buttonVisibilityConfig.allowedRoles || {};
        if (isCurrentDevice) {
    ...
    if ((action === "me" || action === "admin-me") && req.method === "GET") {
      ...
      try {
        ...
        if (decoded.adminType === "subadmin") {
          const session = await SubAdminSession.findOne({ sessionId: decoded.sessionId, isActive: true });
          if (!session || !isSubAdminSessionValid(session)) return res.status(401).json({ success: false, message: "Session ended." });
          await touchSubAdminSession(session);
