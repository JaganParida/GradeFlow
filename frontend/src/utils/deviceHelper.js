...
export function parseDeviceDetails(device = {}) {
  ...
  let os = device.os && device.os !== "Unknown" ? device.os : "Unknown";
  if (os === "Unknown") {
  ...
  let deviceType = device.deviceType && device.deviceType !== "Unknown" ? device.deviceType : "Unknown";
  ...
  // 3. Browser Detection
  let browser = device.browser && device.browser !== "Unknown" ? device.browser : "Unknown";
  if (browser === "Unknown") {
    if (/Edg\//i.test(ua) || (plat && /Edge/i.test(plat))) {
      browser = "Edge";
      ...
      browser = "Firefox";
    } else if (plat && /Chrome/i.test(plat)) {
      browser = "Chrome";
    } else if (plat && /Safari/i.test(plat)) {
      browser = "Safari";
    }
  }

  // 4. Primary Label Construction
  let displayTitle = deviceType !== "Unknown" ? deviceType : "Authorized Device";
  if (os !== "Unknown" && deviceType !== "Unknown") {
    if (os === "iOS" && (/iPhone/i.test(ua) || deviceType === "Mobile")) {
      displayTitle = "iPhone";
    } else if (os === "iPadOS" || /iPad/i.test(ua) || (os === "iOS" && deviceType === "Tablet")) {
      displayTitle = "iPad";
    } else {
  ...
  return {
    displayTitle,
    deviceType: deviceType !== "Unknown" ? deviceType : "Desktop",
    os: os !== "Unknown" ? os : "Desktop OS",
    browser: browser !== "Unknown" ? browser : "Web Browser",
