...
with open('d:/Important/Development_Projects/Advanced/Gradeflow/frontend/src/pages/Dashboard.jsx', 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Replace the headers
old_headers = '''<th colSpan="2" style={{ textAlign: "center", borderBottom: "1px solid var(--border)", borderRight: "1px solid var(--border)", fontSize: 10, color: "var(--secondary)" }}>MID SEMESTER</th>
...
new_headers = '''{internalMarks.semester === 1 ? (
                          <>
...
# 2. Replace the loop for OBTAINED/ROUND OFF
old_loop = '{[...Array(6)].map((_, i) => ('
new_loop = '{[...Array(internalMarks.semester === 1 ? 5 : 6)].map((_, i) => ('
content = content.replace(old_loop, new_loop)

# 3. Replace Desktop mapping array
old_mapping = '''[s.midSemObtained, s.midSemMax, s.midSemRoundOff],
...
new_mapping = '''...(internalMarks.semester === 1 ? [
...
# 4. Replace Mobile mapping array
old_mobile_mapping = '''const mobileAssessments = [
...
new_mobile_mapping = '''const mobileAssessments = internalMarks.semester === 1 ? [
...
with open('d:/Important/Development_Projects/Advanced/Gradeflow/frontend/src/pages/Dashboard.jsx', 'w', encoding='utf-8') as f:
    f.write(content)
